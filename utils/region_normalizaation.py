...
class RN_binarylabel(nn.Module):
    ...
    def rn(self, region, mask):
        ...
        return (
            self.bn_norm(region + (1 - mask) * mu[None, :, None, None])
...
class RN_B(nn.Module):
    def __init__(self, feature_channels):
        ...
        self.foreground_gamma = nn.Parameter(
            torch.zeros(feature_channels), requires_grad=True
        )
    ...
    def forward(self, x, mask):
        ...
        mask = F.interpolate(
            mask, size=x.size()[2:], mode="nearest"
        )  # after down-sampling, there can be all-zero mask

        rn_x = self.rn(x, mask)

        rn_x_foreground = (rn_x * mask) * (
            1 + self.foreground_gamma[None, :, None, None]
        ) + self.foreground_beta[None, :, None, None]
        rn_x_background = (rn_x * (1 - mask)) * (
            1 + self.background_gamma[None, :, None, None]
